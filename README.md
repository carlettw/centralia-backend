...
Stripe qo'shilgandan keyin `Payment` modeliga o'zgarish kiritildi (provider ro'yxatiga
`stripe` qo'shildi, `provider_transaction_id` uzunligi oshirildi). Shuning uchun quyidagini
qayta ishga tushiring:
```bash
alembic revision --autogenerate -m "add stripe provider"
alembic upgrade head
```
